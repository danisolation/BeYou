...
import { CheckCircle, ShieldAlert, Clock, UserCheck, AlertTriangle, Send, Activity, Sparkles } from "lucide-react";
...
export default function StudentSosPage() {
  ...
  useEffect(() => {
    async function loadInitialData() {
      try {
        ...
        const active = alerts.find((alert) => alert.current_status !== "completed");
        if (active) {
  ...
  const getStepClass = (status: string, targetStatus: string, index: number, currentActiveIndex: number) => {
    if (status === targetStatus) return "bg-primary text-white border-primary ring-4 ring-primary/20 scale-110";
    if (index < currentActiveIndex) return "bg-emerald-500 text-white border-emerald-500";
  ...
  const currentStatusIndex = activeAlert ? statusWorkflow.indexOf(activeAlert.current_status) : 0;
    ...
    return (
      <section className="mx-auto max-w-xl space-y-6 py-6 animate-fade-in">
        ...
        <div className="rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/[0.04] to-accent-violet/[0.04] p-6 text-center shadow-sm dark:bg-[#1a2244]">
          ...
          <h1 className="mt-4 text-xl font-bold text-primary dark:text-[#a084ff]">SOS đã được gửi</h1>
        ...
        <div className="rounded-3xl border border-outline-variant/35 bg-white p-6 shadow-sm dark:bg-[#1a2244]">
          ...
          <div className="mt-6 relative flex justify-between items-center px-4">
            ...
            <div 
              ...
              style={{ width: `${(currentStatusIndex / (statusWorkflow.length - 1)) * 100}%` }}
            />
            ...
            {[
              ...
              return (
                <div key={step.id} className="relative z-10 flex flex-col items-center">
                  <div className={`flex h-10 w-10 items-center justify-center rounded-full border-2 transition-all duration-300 bg-white dark:bg-[#0f1530] ${getStepClass(activeAlert.current_status, step.id, idx, currentStatusIndex)}`}>
                    <StepIcon size={18} />
                  </div>
                  <span className={`mt-2 text-xs font-semibold ${isActive ? "text-primary dark:text-[#a084ff]" : isPassed ? "text-emerald-500" : "text-on-background/40"}`}>
                    {step.label}
                  </span>
                </div>
              );
          ...
          <div className="mt-4 space-y-4">
            {activeAlert.status_events.length === 0 ? (
              <div className="rounded-2xl bg-outline-variant/5 p-4 text-center">
                <p className="text-sm text-on-background/60">Tín hiệu đang chờ phản hồi từ người lớn.</p>
                <p className="mt-1 text-xs text-on-background/40">Em hãy ở phòng học, giữ liên lạc hoặc tới văn phòng Đoàn đội/phòng Y tế nhé.</p>
              </div>
            ) : (
              <div className="relative border-l-2 border-primary/20 ml-2.5 pl-5 space-y-4">
                {activeAlert.status_events.map((event) => {
                  ...
                  if (event.new_status === "received") statusText = "Đã nhận tin";
                  if (event.new_status === "supporting") statusText = "Bắt đầu hỗ trợ";
                  if (event.new_status === "completed") statusText = "Hoàn thành hỗ trợ";
                    ...
                    <article key={event.id} className="relative group">
                      ...
                      <div className="rounded-2xl border border-outline-variant/30 bg-primary/[0.02] p-3">
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-semibold text-primary dark:text-[#a084ff]">{roleName} · {statusText}</span>
                          <span className="text-[10px] text-on-background/40">{new Date(event.created_at).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })}</span>
                        </div>
                        {event.note && (
                          <p className="mt-2 text-sm text-on-background/80 bg-white/50 p-2.5 rounded-xl dark:bg-black/10">{event.note}</p>
                        )}
                      </div>
                    </article>
                  );
        ...
        <div className="rounded-3xl border border-outline-variant/35 bg-white p-5 shadow-sm dark:bg-[#1a2244]">
          <h2 className="text-sm font-semibold text-on-background">Tín hiệu đang liên hệ trực tiếp tới:</h2>
          <div className="mt-3 flex flex-wrap gap-2">
            {selectedAdults.length === 0 ? (
              ...
              selectedAdults.map((adult) => (
                <div key={adult.id} className="flex items-center gap-1.5 rounded-2xl bg-primary/5 border border-primary/10 px-4.5 py-2 text-xs font-semibold">
                  <span className="h-2 w-2 rounded-full bg-primary" />
                  <span>{adult.full_name} ({adult.relationship_type === "teacher" ? "Giáo viên" : "Phụ huynh"})</span>
                </div>
              ))
            )}
          </div>
        ...
        <div className="rounded-3xl border border-outline-variant/35 bg-white p-5 shadow-sm dark:bg-[#1a2244]">
          <h2 className="text-sm font-semibold text-on-background">Đường dây nóng khẩn cấp bên ngoài:</h2>
          <ul className="mt-3 space-y-1.5 text-xs text-on-background/70">
            <li>📞 <strong>Tổng đài Quốc gia Bảo vệ Trẻ em:</strong> <span className="text-primary font-bold">111</span> (Miễn phí)</li>
            <li>🚑 <strong>Cấp cứu y tế:</strong> <span className="text-primary font-bold">115</span></li>
          </ul>
        </div>
    ...
    <section className="mx-auto max-w-lg space-y-6 py-6 animate-fade-in">
      <div className="rounded-3xl border border-outline-variant/30 bg-white dark:bg-[#1a2244] p-6 text-center shadow-sm">
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-error-container">
          <ShieldAlert size={32} className="text-error animate-bounce" aria-hidden="true" />
        </div>
        <h1 className="mt-4 text-lg font-bold text-on-background">Hỗ trợ khẩn cấp</h1>
      ...
      <div className="rounded-3xl border border-outline-variant/30 bg-white dark:bg-[#1a2244] p-5 space-y-3 shadow-sm">
        <h2 className="text-sm font-semibold text-on-background">Mức độ hỗ trợ em đang cần:</h2>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          >
            ...
            <span className="mt-1 text-[11px] text-on-background/60 leading-tight">
              Em an toàn nhưng cần người em tin liên hệ chia sẻ trong thời gian sớm nhất.
            </span>
          </button>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-outline-variant/30 bg-white dark:bg-[#1a2244] p-5 shadow-sm">
        <h2 className="text-sm font-semibold text-on-background">Khi em nhấn gửi SOS:</h2>
        ...
        {selectedAdults.length > 0 && (
          <div className="mt-4 pt-3.5 border-t border-outline-variant/20">
            <p className="text-[11px] font-semibold text-on-background/50">Tín hiệu sẽ được gửi tới danh sách đã chọn:</p>
            <div className="mt-2 flex flex-wrap gap-1.5">
              {selectedAdults.map((adult) => (
                <span key={adult.id} className="inline-flex items-center gap-1 rounded-xl bg-outline-variant/15 px-3 py-1 text-[10px] font-bold text-on-background/70">
                  🧑‍🤝‍🧑 {adult.full_name}
                </span>
              ))}
            </div>
