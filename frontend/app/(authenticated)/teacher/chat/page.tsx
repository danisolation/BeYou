...
const INTRO_COPY =
  "Peerlight AI hỗ trợ bạn suy nghĩ về cách đồng hành cùng học sinh. Nội dung trò chuyện riêng tư và không được chia sẻ với học sinh.";
...
export default function TeacherChatPage() {
  ...
  useEffect(() => {
    ...
    listTeacherChatThreads()
      .then(async (threads) => {
        ...
        const transcript = await getTeacherChatTranscript(latest.id);
        if (active) {
  ...
  async function handleSelectThread(nextThreadId: string) {
    ...
    setError("");
    try {
      const transcript = await getTeacherChatTranscript(nextThreadId);
      setThreadId(transcript.thread.id);
      setMessages(transcript.messages);
    } catch {
  ...
  async function handleSend(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      const response = await sendTeacherChatMessage({ message, thread_id: threadId });
      ...
      <div className="grid gap-4 md:grid-cols-[16rem_1fr]">
        ...
        <section className="flex flex-col rounded-2xl border border-outline-variant/30 bg-white dark:bg-[#1a2940]">
          ...
          <div className="flex items-center gap-3 border-b border-outline-variant/20 px-4 py-3">
            ...
            <h2 className="text-sm font-semibold text-on-background">Peerlight AI – Hỗ trợ giáo viên</h2>
          </div>
          ...
          <form className="border-t border-outline-variant/20 px-4 py-3" onSubmit={handleSend}>
            <div className="flex items-end gap-2">
              <textarea
                ...
                placeholder="Hỏi về cách hỗ trợ học sinh..."
                rows={2}
