    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-base text-on-background/70">
          {students.length} con đang được đồng hành
        </p>
      </div>

      {students.length === 0 ? (
        <div className="rounded-2xl border border-outline-variant/30 bg-white dark:bg-[#1a2244] p-8 text-center dark:bg-[var(--surface)]">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {students.map((s) => (
            <Link
              key={s.id}
              href={`/parent/students/${s.id}/support-summary`}
              className="flex items-center gap-4 rounded-[24px] border border-outline-variant/30 bg-white dark:bg-[#1a2244] p-5 no-underline transition-all hover:-translate-y-0.5 hover:shadow-md dark:bg-[var(--surface)]"
            >
