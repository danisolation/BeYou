...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_external_identities_linked_user_id", "external_identities", ["linked_user_id"]
    )
    ...
    op.add_column(
        "sessions", sa.Column("external_identity_id", postgresql.UUID(as_uuid=True), nullable=True)
    )
...
def downgrade() -> None:
    op.drop_constraint(
        "fk_sessions_external_identity_id_external_identities", "sessions", type_="foreignkey"
    )
